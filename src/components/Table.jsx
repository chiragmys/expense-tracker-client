...
export default function ExpenseTable({allExpenses,fetchAllExpenses}) {
  const handleDelete=async(expenseId)=>{
    try {
      const res=await axios.delete(`${baseur1}/api/expense/delete/${expenseId}`)
        ...
        <TableBody>
          ...
          allExpenses.map((row,index) => (
            ...
            >
              ...
              <TableCell><Button component={Link} to={`edit/${row._id}`} color='info' variant='contained' >Edit</Button>
              <Button onClick={() =>handleDelete(row?._id)} color='error' variant='contained'>Delete</Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
